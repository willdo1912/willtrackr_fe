...
const Login = () => {
  ...
  const [loginInput, setLoginInput] = useState<LoginInputModel>({
    email: '',
    password: ''
  })
  ...
  const handleLoginSubmit = (e: FormEvent<HTMLFormElement>) => {
    setErrorMsg('')
    e.preventDefault()
    for (let key in loginInput) {
      if (loginInput[key as keyof LoginInputModel].trim() === '') {
        setErrorMsg(`${key} cannot be empty`)
        return
      }
    }
    // console.log(snakelize(loginInput))
    axios
      .post(`${import.meta.env.VITE_BE_PATH}/api/customer/login/`, snakelize(loginInput))
      .then(response => response.data)
      .then(data => window.localStorage.setItem('id', data.id))
      .then(() => setErrorMsg(''))
  ...
  function handleInput(inputName: string, inputValue: any): void {
    const newInput = { ...loginInput, [inputName]: inputValue }
    setLoginInput(newInput)
  }
  return (
    <div className="flex h-dvh flex-row items-center justify-center bg-[url('https://images.pexels.com/photos/1128678/pexels-photo-1128678.jpeg')] bg-cover bg-right bg-no-repeat">
      <div className="absolute z-0 size-full bg-transparent opacity-0 dark:bg-black dark:opacity-90"></div>
      <form
        className="base-bg-normal custom-shadow-outer z-10 flex flex-col items-center justify-center gap-4 rounded-2xl p-6 md:gap-8 md:p-12"
      ...
      >
        ...
        <button className="btn-filled inline-flex h-12 w-full items-center justify-center">Login</button>
